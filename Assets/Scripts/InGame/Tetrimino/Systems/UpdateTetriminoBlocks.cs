...
namespace Tetris
{
    ...
    public partial struct UpdateTetriminoBlocksSystem : ISystem
    {
        [BurstCompile]
        [WithChangeFilter(typeof(TetriminoData))]
        public partial struct UpdateTetriminoBlocksJob : IJobEntity
        {
            public ComponentLookup<BlockPosition> positionLookup;
            public ComponentLookup<URPMaterialPropertyBaseColor> colorLookup;

            [BurstCompile]
            private void Execute(in TetriminoData data, in DynamicBuffer<TetriminoBlockBuffer> blocks)
            {
